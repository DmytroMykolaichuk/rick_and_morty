export async function getInitialPages(endpoint:string,page: number){
    const {results} = await fetch(`https://rickandmortyapi.com/api/${endpoint}/?page=${page}`).then(response => response.json());
    return results
}
...
export  async function getLocation(id: number) {
    const {  name,dimension,type,residents } = await fetch(`https://rickandmortyapi.com/api/location/${id}`).then(response => response.json());
